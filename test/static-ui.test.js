...
const publicRuntimeEnvKeys = [
  ...
  "GOOGLE_OAUTH_APPS_JSON",
  "ORKESTR_GOOGLE_OAUTH_DEFAULT_APP",
  "GOOGLE_OAUTH_DEFAULT_APP",
...
test("web shell exposes an instance-scoped desktop page", async () => {
  ...
  assert.match(template, /<ork-user-desk-page \[threads\]="threads" \[selectedThread\]="selectedThread\(\)"><\/ork-user-desk-page>/);
...
test("mobile desktop shell wraps noVNC with phone-first controls", async () => {
  ...
  assert.match(proxy, /resolveDesktopTarget/);
  assert.doesNotMatch(proxy, /ensureVirtualBrowserReady/);
