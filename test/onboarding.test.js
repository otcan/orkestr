...
test("onboarding focuses the starter setup on virtual desktop and WhatsApp", async () => {
  ...
  assert.ok(onboarding.includes("Sign-in expired. Open Codex sign-in again."));
  ...
  assert.ok(onboarding.includes('retiredSetupSections = new Set(["google-marketing", "openai", "linkedin", "mail", "outlook"])'));
  assert.ok(onboarding.includes('return ["system", "security", "secrets", "maintenance", "codex", "gmail", "whatsapp", "browsers"].includes(section)'));
  ...
  assert.ok(onboarding.includes("prepareRestorePlan()"));
  assert.ok(onboarding.includes("runCodexMigration(false)"));
  assert.ok(onboarding.includes("backupRestorePlan"));
