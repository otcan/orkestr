...
test("release deploy script exposes versioned install, status, and rollback", async () => {
  ...
  assert.match(script, /ORKESTR_DEPLOY_BACKUP_COMPRESSOR/);
  assert.match(script, /ORKESTR_DEPLOY_BACKUP_GZIP_LEVEL/);
  ...
  assert.match(script, /compressor_cmd=\(pigz "-\$gzip_level" -c\)/);
  assert.match(script, /compressor_cmd=\(gzip "-\$gzip_level" -c\)/);
  assert.match(script, /State backup started: \$backup_name using \$compressor/);
  assert.match(script, /State backup completed in \$\{elapsed\}s/);
  ...
  assert.match(script, /find "\$backup_dir" -maxdepth 1 -type f \\\( -name '\*\.tar\.gz' -o -name '\*\.tar\.zst' -o -name '\*\.tar' \\\)/);
  assert.match(script, /State backup completed with non-fatal live-file changes/);
  assert.match(script, /ORKESTR_RELEASE_WHATSAPP_ACCOUNT_PROBE_TIMEOUT_SECONDS/);
  assert.match(script, /probe_timeout="\$\(positive_integer_env "\$\{ORKESTR_RELEASE_WHATSAPP_ACCOUNT_PROBE_TIMEOUT_SECONDS:-\}" 8 1\)"/);
  assert.match(script, /timeout "\$\{probe_timeout\}s" env ORKESTR_API_BASE="\$api_base"/);
  assert.match(script, /ORKESTR_RELEASE_CONNECTIVITY_RECOVERY_TIMEOUT_SECONDS/);
  assert.match(script, /recovery_timeout="\$\(positive_integer_env "\$\{ORKESTR_RELEASE_CONNECTIVITY_RECOVERY_TIMEOUT_SECONDS:-\}" 20 1\)"/);
  assert.match(script, /timeout "\$\{recovery_timeout\}s" env/);
  ...
  assert.match(script, /health_check "\$health_url" 40 \|\| return \$\?\s+restart_and_verify_public_service \|\| return \$\?\s+restart_and_verify_mailbox_mta \|\| return \$\?\s+sync_standalone_connectors_release \|\| return \$\?\s+refresh_parent_whatsapp_bridge_proxy \|\| return \$\?\s+deploy_public_exposure_check \|\| return \$\?/);
  assert.match(script, /ORKESTR_RELEASE_SYNC_CONNECTORS/);
  assert.match(script, /ORKESTR_RELEASE_SYNC_PUBLIC_SERVICE/);
  assert.match(script, /ORKESTR_RELEASE_SYNC_PUBLIC_SERVICE must be 0 or 1/);
  assert.match(script, /ORKESTR_PUBLIC_SERVICE_NAME/);
  assert.match(script, /ORKESTR_PUBLIC_SERVICE_HEALTH_URL/);
  assert.match(script, /ORKESTR_PUBLIC_SERVICE_VERSION_URL/);
  assert.match(script, /restart_and_verify_public_service/);
  assert.match(script, /systemctl is-active --quiet "\$public_unit"/);
  assert.match(script, /WorkingDirectory --value "\$public_unit"/);
  assert.match(script, /working_directory" != "\$current_link"/);
  assert.match(script, /systemctl restart "\$public_unit"/);
  assert.match(script, /process cwd '\$process_cwd' does not match '\$expected_release'/);
  assert.match(script, /actual_revision" != "\$expected_revision"/);
  assert.match(script, /restart_and_verify_public_service \|\| return \$\?/);
  assert.match(script, /ORKESTR_RELEASE_SYNC_MAILBOX_MTA/);
  assert.match(script, /ORKESTR_RELEASE_SYNC_MAILBOX_MTA must be 0 or 1/);
