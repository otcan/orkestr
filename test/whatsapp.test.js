...
test("local WhatsApp bridge rejects oversized media before sending cover text", async () => {
  ...
  try {
    setLocalWhatsAppRuntimeForTest("responder", runtime, {}, env);
    await assert.rejects(sendLocalWhatsAppMessage({
      accountId: "responder",
...
test("local WhatsApp delivery fails preflight rather than claiming text-only success", async () => {
  ...
  try {
    setLocalWhatsAppRuntimeForTest("responder", runtime, {}, env);
    await assert.rejects(sendWhatsAppText({
      accountId: "responder",
      chatId: "oversized-media@g.us",
      text: "Video export is ready.",
      attachments: [{ path: attachmentPath, filename: "oversized.mp4", mimetype: "video/mp4" }],
      env,
    }), /whatsapp_attachment_preflight_failed/);
    assert.equal(sent.length, 0);
  } finally {
    await resetLocalWhatsAppBridgeForTest(env);
  }
...
test("local whatsapp exact recovery resolves a bare event id from the scoped browser chat", async () => {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "orkestr-wa-media-bare-exact-recovery-"));
  const env = {
    ORKESTR_HOME: home,
    ORKESTR_WHATSAPP_ACCOUNT_IDS: "sender",
    ORKESTR_WHATSAPP_INBOUND_MEDIA_DOWNLOAD_ATTEMPTS: "1",
  ...
  const chatId = "chat-media-bare-exact-recovery@g.us";
  const eventId = "3EB0BAREEVENT123";
  let browserCalls = 0;
  const client = {
    async getMessageById(id) {
      assert.equal(id, eventId);
    ...
    pupPage: {
      async evaluate(_callback, id, expectedChatId) {
        browserCalls += 1;
        assert.equal(id, eventId);
        ...
        return {
          data: Buffer.from("bare exact candidate cv").toString("base64"),
          filename: "bare-exact-candidate.docx",
          mimetype: "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        };
      },
    },
  ...
  await createThread({
    id: "media-bare-exact-recovery-thread",
    name: "Media bare exact recovery",
    binding: {
      connector: "whatsapp",
      chatId,
      responderAccountId: "sender",
  ...
  try {
    ...
    const messages = await listThreadMessages("media-bare-exact-recovery-thread", env);
    const attachment = messages.at(-1)?.attachments?.[0];

    assert.equal(result.routed.length, 1);
    assert.equal(browserCalls >= 2, true);
    assert.equal(attachment.filename, "bare-exact-candidate.docx");
    assert.equal(await fs.readFile(attachment.path, "utf8"), "bare exact candidate cv");
  } finally {
    await resetLocalWhatsAppBridgeForTest(env);
  }
});
...
test("whatsapp bridge injection ignores cross-account outbound text echoes", async () => {
  ...
  const priorDefaultResponder = process.env.ORKESTR_WHATSAPP_DEFAULT_RESPONDER_ACCOUNT_ID;
  process.env.ORKESTR_HOME = home;
  process.env.ORKESTR_AUTH_REQUIRED = "1";
  ...
  try {
    ...
    if (priorDefaultResponder === undefined) delete process.env.ORKESTR_WHATSAPP_DEFAULT_RESPONDER_ACCOUNT_ID;
    else process.env.ORKESTR_WHATSAPP_DEFAULT_RESPONDER_ACCOUNT_ID = priorDefaultResponder;
  }
});
...
test("whatsapp inbound marks Codex input for active-turn steer when explicitly configured", async () => {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "orkestr-wa-explicit-steer-"));
  const env = externalBridgeEnv(home, { ORKESTR_WHATSAPP_INBOUND_STEER_DEFAULT: "1" });
  await createThread({
    id: "thread-wa-explicit-steer",
    name: "WA Explicit Steer Thread",
    runtimeKind: "codex-app-server",
    binding: { connector: "whatsapp", chatId: "chat-explicit-steer", enabled: true },
  }, env);
  await writeConnectorConfig("whatsapp", {
    bridgeMode: "external",
    bridgeUrl: "http://wa.local",
  ...
  const routed = await routeWhatsAppInbound({
    eventId: "wa-explicit-steer-1",
    chatId: "chat-explicit-steer",
    accountId: "account-1",
    text: "incorporate this clarification now",
  }, env);
