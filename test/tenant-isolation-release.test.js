...
test("tenant isolation release suite is named and wired into the release train", async () => {
  ...
  assert.doesNotMatch(checklist, /private-(?:domain|host)|real-chat-id|@g\.us/);
  ...
  assert.doesNotMatch(containmentMatrix, /private-(?:domain|host)|real-chat-id|@g\.us/);
  ...
  assert.doesNotMatch(routeSecurityMatrix, /private-(?:domain|host)|real-chat-id|@g\.us/);
});
