...
test("Claude YOLO MCP allowlist is server-configured and explicit", () => {
  const env = {
    ...
    ORKESTR_CLAUDE_CODE_YOLO_ALLOWED_MCP_TOOLS: "mcp__atlassian__*,mcp__docs__search,mcp__atlassian__*",
  };
  assert.deepEqual(claudeCodeYoloAllowedMcpTools(env), ["mcp__atlassian__*", "mcp__docs__search"]);
  const args = claudeCodeArgs(yoloThread, {}, env);
  const allowedAt = args.indexOf("--allowedTools");
  assert.notEqual(allowedAt, -1);
  assert.equal(args[allowedAt + 1], "mcp__atlassian__*,mcp__docs__search");
});

test("Claude MCP allowlist is not applied outside YOLO mode", () => {
  const args = claudeCodeArgs({ claudePermissionMode: "acceptEdits" }, {}, {
    ORKESTR_CLAUDE_CODE_YOLO_ALLOWED_MCP_TOOLS: "mcp__atlassian__*",
  });
...
test("Claude YOLO MCP allowlist rejects malformed or overbroad tool expressions", () => {
  ...
  for (const configured of ["*", "Bash(*)", "mcp__atlassian__tool with spaces", "mcp__atlassian__read,*"]) {
    assert.throws(
