...
test("share expiry fences an established noVNC socket without waiting for periodic revalidation", async () => {
  const previousHome = process.env.ORKESTR_HOME;
  const previousPublicUrl = process.env.ORKESTR_PUBLIC_HTTPS_URL;
  process.env.ORKESTR_HOME = await fs.mkdtemp(path.join(os.tmpdir(), "orkestr-desktop-proxy-expiry-"));
  process.env.ORKESTR_PUBLIC_HTTPS_URL = "https://app.example.test";
  try {
    ...
    const { registerDesktopShareSocket } = await import("../dist/server/apps/server/src/desktop-proxy.js");
    const principal = { kind: "user", userId: "alice", role: "user", source: "test" };
    const created = await createDesktopShare({ desktopSlug: "linkedin", principal });
    const parts = urlParts(created.url);
    const opened = await openDesktopShare({ shareId: parts.shareId, key: parts.key, subdomain: created.subdomain });
    const approved = await approveDesktopShareChallenge(opened.attempt.challenge, { approvedBy: "test" });
    const expiresAt = new Date(Date.now() + 500).toISOString();
    const statePath = path.join(process.env.ORKESTR_HOME, "secrets", "desktop-shares.json");
    const state = JSON.parse(await fs.readFile(statePath, "utf8"));
    state.desktopShares[0].expiresAt = expiresAt;
    await fs.writeFile(statePath, `${JSON.stringify(state)}\n`);
    approved.share.expiresAt = expiresAt;
    const socket = new PassThrough();
    const upstream = new PassThrough();
    registerDesktopShareSocket(socket, upstream, approved.share, approved.attempt);
