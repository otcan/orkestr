...
import { appendThreadMessage, createThread, getThread, listThreadMessages, updateThreadMessage } from "../packages/core/src/threads.js";
...
import { markConnectorOutboxJob, readConnectorOutbox } from "../packages/connectors/src/connector-outbox.js";
...
for (const phase of ["final_answer", "commentary"]) for (const failure of ["partial_delivery", "delivery_uncertain"]) test(`${phase}: staged snapshot never reopens ${failure}`, async t => {
  ...
  await deliverWhatsAppReplies(env, async (_url, options = {}) => {
    if (options.method !== "POST") return new Response(JSON.stringify({ ok: true, ready: true }), {
      headers: { "content-type": "application/json" },
    });
    sends++;
    if (failure === "delivery_uncertain") throw new Error("whatsapp_send_not_confirmed");
    return new Response(JSON.stringify({ ok: false, error: "whatsapp_partial_delivery", partialDelivery: {
      sent: [{ id: "synthetic-text", kind: "text" }], attachments: [{ index: 0, outcome: "uncertain" }],
      failedKind: "attachment", failureCode: "provider_rejected", stage: "send_media",
    } }), { status: 409, headers: { "content-type": "application/json" } });
  });
  assert.equal(sends, 1);
  const stored = (await listThreadMessages(thread.id, env)).find(item => item.id === reply.id);
  assert.equal(stored.deliveryState, failure);
  assert.equal(requiredOutboundSnapshots(stored.attachments).length, 1);
  await deliverWhatsAppReplies(env, async () => { sends++; throw new Error("must never replay"); });
  assert.equal(sends, 1);
  const jobs = (await readConnectorOutbox(env)).jobs.filter(job => job.sourceMessageId === reply.id);
  assert.equal(jobs.length, 1);
  assert.equal(jobs[0].state, failure);
});

test("claimed retry enforces snapshot obligations retained only in the durable outbox", async t => {
  const { env, thread, source } = await fixture(t);
  env.ORKESTR_CONNECTOR_OUTBOX_RETRY_BACKOFF_MS = "0";
  env.ORKESTR_TEST_RUNTIME_FAULT_INJECTOR = { transport_send: () => { throw new Error("synthetic pretransport pause"); } };
  const parent = await appendThreadMessage(thread.id, { role: "user", connector: "whatsapp", source: "whatsapp_inbound",
    chatId: "synthetic-chat", text: "Document please", state: "completed" }, env);
  const reply = await appendThreadMessage(thread.id, { role: "assistant", connector: "whatsapp", source: "codex-app-server",
  ...
  const job = (await readConnectorOutbox(env)).jobs.find(item => item.sourceMessageId === reply.id);
  assert.equal(job.state, "failed_retryable");
  const extraSource = path.join(path.dirname(source), "additional.txt");
  await fs.writeFile(extraSource, "synthetic additional obligation");
  const [extra] = await snapshotRoutedAttachments({ thread, message: reply, attachments: [{ path: extraSource }], env });
  await markConnectorOutboxJob(job.id, { payload: { ...job.payload,
    requiredAttachmentSnapshots: [...job.payload.requiredAttachmentSnapshots, ...requiredOutboundSnapshots([extra])],
  } }, env);
  delete env.ORKESTR_TEST_RUNTIME_FAULT_INJECTOR;
  const result = await deliverWhatsAppReplies(env, forbidden);
  assert.equal(sends, 0);
  assert.equal(result.failed.length, 1);
  assert.match(result.failed[0].error, /snapshot_not_sendable/);
});

for (const phase of ["final_answer", "commentary"]) for (const encrypted of [false, true]) test(`${phase}: routed document survives original removal with encryption=${encrypted}`, async t => {
  ...
  const beforeFinalDelivery = (await getThread(thread.id, env)).runtime?.finalDelivery;
  const beforeParent = (await listThreadMessages(thread.id, env)).find(item => item.id === parent.id);
  const calls = [];
  ...
  if (phase === "commentary") {
    assert.deepEqual((await getThread(thread.id, env)).runtime?.finalDelivery, beforeFinalDelivery);
    const afterMessages = await listThreadMessages(thread.id, env);
    assert.deepEqual(afterMessages.find(item => item.id === parent.id), beforeParent);
    assert.equal(afterMessages.find(item => item.id === reply.id).deliveryState, "delivered");
  }
...
for (const phase of ["final_answer", "commentary"]) for (const encrypted of [false, true]) test(`${phase}: missing snapshot remains retryable until restored with encryption=${encrypted}`, async t => {
  const { env, thread, source, bytes } = await fixture(t);
  env.ORKESTR_CONNECTOR_OUTBOX_RETRY_BACKOFF_MS = "0";
  if (encrypted) await enableEncryption(env);
  const parent = await appendThreadMessage(thread.id, { role: "user", connector: "whatsapp", source: "whatsapp_inbound",
