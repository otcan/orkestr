...
test("claudeCodeAccountDiagnostics probes with a configured long-lived subscription token like login verification", async (t) => {
  const dir = await tmpDir(t);
  const command = path.join(dir, "fake-claude-token.mjs");
  await fs.writeFile(command, `#!/usr/bin/env node
const args = process.argv.slice(2);
if (args[0] === "auth" && args[1] === "status") {
  const tokenSeen = Boolean(process.env.CLAUDE_CODE_OAUTH_TOKEN);
  process.stdout.write(JSON.stringify({ authenticated: tokenSeen, status: tokenSeen ? "logged_in" : "logged_out" }));
  process.exit(0);
}
process.exit(1);
`, { mode: 0o700 });
  t.after(() => resetClaudeCodeDiagnosticsForTest());
  const env = { ORKESTR_HOME: dir, ORKESTR_CLAUDE_CODE_BIN: command };
  const profile = await createLlmAccountProfile("owner", { provider: "claude-code", label: "Token diag" }, env);
  await setClaudeSubscriptionToken("owner", profile.id, `sk-ant-oat01-${"a".repeat(40)}`, env);
  const resolved = await resolveLlmAccountProfile({ ownerUserId: "owner", profileId: profile.id, provider: "claude-code", requireReady: false }, env);
  const diag = await claudeCodeAccountDiagnostics(resolved, env);
  assert.equal(diag.authenticated, true);
  assert.doesNotMatch(JSON.stringify(diag), /sk-ant-oat01/);
});
