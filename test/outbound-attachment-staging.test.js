...
async function fixture(t) {
  ...
  const reply = () => appendThreadMessage(thread.id, { role: "assistant", source: "codex-app-server", connector: "whatsapp", chatId: "synthetic-chat",
    parentMessageId: parent.id, state: "completed", phase: "final_answer", text: `Document: [workbook](${source})` }, env);
  return { home, env, thread, source, reply };
}

for (const fault of ["missing_source", "copy_failure"]) test(`initial ${fault} preserves reply and durable retry across reload`, async t => {
