...
const contextShape = {
  ...
  resource_type: z.enum(["desktop", "oxrm", "mailbox"]).nullish().describe("Optional registered resource type. When supplied in enforce mode, an exact resource-bound bearer claim is required."),
  resource_id: cleanString.nullish().describe("Canonical registered resource id. Resource-bound calls never infer this from connector account or instance fields."),
  resource_action: cleanString.nullish().describe("Exact resource permission required by a resource-bound call, such as read, execute, or operate."),
