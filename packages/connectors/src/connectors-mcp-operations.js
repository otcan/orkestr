...
import {
  connectorMcpChallengeResult,
  requireConnectorMcpApproval,
} from "./connectors-mcp-approval.js";
...
export async function runConnectorMcpTool(tool = "", rawInput = {}, { auth = {}, request = null, env = process.env } = {}) {
  ...
  const challenge = await requireConnectorMcpApproval(tool, input, scoped, request, env);
  if (challenge) return connectorMcpChallengeResult(input, challenge, scoped);
  try {
