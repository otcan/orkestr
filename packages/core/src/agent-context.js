...
- For contained users, \`whereiam.capabilities.enabledSkills\` is the allowed
  ...
  fallbacks.
- For tenant VMs, user-facing browser and WebUI instructions must use
  \`whereiam.publicUrls.appUrl\` or \`ORKESTR_PUBLIC_URL\`. Setup or pairing
  recovery instructions must use \`whereiam.publicUrls.setupUrl\`,
  \`ORKESTR_CONNECT_PUBLIC_SETUP_URL\`, or \`ORKESTR_PAIRING_URL\`. Treat
  \`0.0.0.0\`, \`127.0.0.1\`, \`localhost\`, private \`10.x\` /
  \`172.16-31.x\` / \`192.168.x\` addresses, and raw
