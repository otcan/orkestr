...
import {
  deployReleaseInstancesWithResolver,
  releaseCommandConfigured,
} from "./release-instance-deployment.js";
...
function normalizeVersionPayload(payload = {}) {
  ...
  const version = {
    ...
    releaseLabel: clean(payload.releaseLabel),
    releaseVersion: clean(payload.releaseVersion || payload.version),
    buildId: clean(payload.buildId || payload.releaseId),
...
export function publicReleaseInstance(instance = {}) {
  ...
  return {
    ...
    hasDeployCommand: releaseCommandConfigured(normalized.deployCommand),
    hasConnectivityCommand: releaseCommandConfigured(normalized.connectivityCommand),
    hasConnectivityRecoveryCommand: releaseCommandConfigured(normalized.connectivityRecoveryCommand),
...
export async function deployReleaseInstances(options = {}, env = process.env) {
  return deployReleaseInstancesWithResolver({ listReleaseInstances, normalizeReleaseInstance }, options, env);
}
