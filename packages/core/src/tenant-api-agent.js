...
function formatToolResultFallback(toolResults = [], context = {}) {
  ...
  for (const result of toolResults) {
    ...
    else if (result.name === "orkestr_list_actions" || result.name === "orkestr_list_action_registry") formatted = formatActionRegistryTool(result);
    else if (result.name === "orkestr_run_action") {
      const output = result.output || {};
      if (output.result && output.handler) {
        const legacyResult = { name: output.handler, output: output.result };
        if (["orkestr_search_gmail", "orkestr_read_gmail_message", "orkestr_read_latest_gmail_message"].includes(output.handler)) formatted = formatGmailTool(legacyResult);
...
export async function buildTenantApiAgentInstructions(thread = {}, messages = [], env = process.env) {
  ...
  return [
    ...
    "When Gmail capability is true and the user asks to search, list, open, read, inspect, or summarize Gmail, use orkestr_list_actions and orkestr_run_action for the matching Gmail action. The user's request is consent for that same-user Gmail action; do not ask for repeated confirmation unless the target email or search is ambiguous.",
    "When Gmail capability is true and the Tenant connectorAuth.gmail capabilities include Gmail actions, Gmail send, Calendar read, Calendar actions, or Drive selected files, use orkestr_list_actions and orkestr_run_action for the matching action. For sending email, require explicit approval of recipients, subject, and body before sending; drafts may be created when the user asks to draft. For Calendar actions, require explicit approval of the calendar, title, time, and changed/deleted event before creating, updating, or deleting events.",
    "When Gmail capability is true and the user asks to notify, alert, push, monitor, or periodically check new Gmail in this chat, call orkestr_run_action for a Gmail notification/watch action in that turn to create or manage the rule. Do not ask for yes/no confirmation when the safe defaults are enough; the user's request is consent for a same-chat notification rule. Use the safe default query when the user did not specify a narrower query, and explain after the tool result if the requested interval was rounded up by policy.",
