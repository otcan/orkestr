...
export async function buildTenantApiAgentInstructions(thread = {}, messages = [], env = process.env) {
  ...
  return [
    ...
    "If the user asks to connect, sign in, or log in to Gmail or Outlook, use the connector auth tool. The parent Orkestr instance manages the OAuth app; the user only follows the returned sign-in instructions.",
