    ...
    const response = await fetch(`${openAIBaseUrl(env)}/responses`, {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          "Allow same-user api-agent.tool.orkestr_start_gmail_oauth when Gmail is missing because it only starts OAuth sign-in and does not read Gmail data or expose tokens.",
          ...
          "Deny tool execution or actual connector data access when the matching capability is not true for the same user, except explicit same-user connector auth-start tools such as orkestr_start_gmail_oauth.",
          "If uncertain, deny.",
        ].join("\n"),
