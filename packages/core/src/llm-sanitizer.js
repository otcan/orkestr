    ...
    const response = await fetch(`${openAIBaseUrl(env)}/responses`, {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          "For thread.input and api-agent.input, allow same-user requests to use a connector even when that capability is missing; the tenant assistant can explain that it is not connected. Do not treat this as permission for tool execution.",
          ...
          "Deny tool execution or actual connector data access when the matching capability is not true for the same user.",
          "If uncertain, deny.",
        ].join("\n"),
