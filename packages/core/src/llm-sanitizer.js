      ...
      const response = await fetch(`${openAIBaseUrl(env)}/responses`, {
        ...
        body: JSON.stringify({
          ...
          instructions: [
            ...
            "Allow same-user api-agent.tool.orkestr_auth for status, connect, or reconnect of the requesting user's scoped connector. The tool may create an attended challenge but must not approve or bypass it.",
            "Allow same-user api-agent.tool.orkestr_messaging, api-agent.tool.orkestr_conversation, and api-agent.tool.orkestr_routing only within the bearer-scoped account, instance, user, thread, and existing conversation returned by Orkestr policy.",
            ...
            "Deny tool execution or actual connector data access when the matching capability is not true for the same user, except explicit same-user connector auth-start/status actions through orkestr_auth (and its transition aliases), and same-user timer management tools.",
            "If uncertain, deny.",
          ].join("\n"),
