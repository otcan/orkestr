...
async function authorizeBrokerProxyMachineRequest(request, env = process.env) {
  ...
  const payloadDisplayName = String(payload.displayName || payload.name || payloadUserId || "").trim();
  const principal = payloadUserId && payloadRole === "user"
    ? userPrincipal({ id: payloadUserId, role: "user", source: "broker-proxy-machine-token", displayName: payloadDisplayName || payloadUserId })
...
function isAllowedBeforePairing(request) {
  ...
  if (method === "POST" && /^\/api\/shared-apps\/i\/[^/]+\/a\/[^/]+\/s\/[^/]+\/pair$/.test(url)) return true;
