...
function lifetimeMinutes(env) {
  const configured = Number(env.ORKESTR_GOOGLE_WORKSPACE_REVIEW_ENV_TTL_MINUTES || 240);
  return Math.max(5, Math.min(1440, Number.isFinite(configured) ? configured : 240));
}
...
export function reviewerBrowserSessionFields(userId, env = process.env) {
  ...
  return { authProvider: "google_workspace_review", reviewBinding, expiresAt: new Date(Date.now() + lifetimeMinutes(env) * 60_000).toISOString() };
}
...
export function reviewerBrowserSessionActive(session, env = process.env) {
  if (session.authProvider !== "google_workspace_review") {
    ...
    // Disabling the feature must not revive old untagged password sessions.
    // The reviewer identity remains reserved while its config is retained.
    return !Object.hasOwn(session, "reviewBinding") && clean(session.userId) !== reviewerUser(env);
  }
