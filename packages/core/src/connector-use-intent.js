 ...
 * @param {string} userId
 * @param {{ connector: string; purpose: string; host?: string }} [options]
 * @param {NodeJS.ProcessEnv} [env]
 * @returns {Promise<{ intentId: string; token: string }>}
 */
export async function createConnectorUseIntent(userId, { connector, purpose, host = "" } = {}, env = process.env) {
 ...
 * @param {string} token
 * @param {{ userId: string; connector: string; purpose: string; host?: string }} [options]
 * @param {NodeJS.ProcessEnv} [env]
 * @returns {Promise<Record<string, unknown>>}
 */
export async function consumeConnectorUseIntent(intentId, token, { userId, connector, purpose, host = "" } = {}, env = process.env) {
  ...
  if (rejectReason === "not_found") {
    // Emit no audit event for not_found (intentId is completely unknown to this user; could be probing).
