...
const ASSISTANT_PHASES = new Set(["commentary", "final_answer", "plan", "need_input"]);
...
function safeMirrorMessage(call, message) {
  ...
  if (!sourceMessageId || !text || !["user", "assistant"].includes(role)) return null;
  if (role === "assistant" && !assistantMessage(message)) return null;
  const rawPhase = clean(message?.phase || (role === "assistant" ? "final_answer" : "input")).toLowerCase();
  if (role === "assistant" && !ASSISTANT_PHASES.has(rawPhase)) return null;
  ...
  return {
    ...
    message: {
      ...
      role,
      phase: role === "user" ? "input" : rawPhase,
      origin: clean(message?.source).toLowerCase() === "hush" ? "hush" : "thread",
      text,
...
/**
 * Copies only the safe, visible messages created after this call began into
