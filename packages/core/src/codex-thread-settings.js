...
export function resolveCodexThreadSettingsCommand({ command, text = "", thread = {}, models: inputModels = [] } = {}) {
  ...
  const explicitCurrentModel = thread.codexModel || thread.executor?.metadata?.codexModel;
  const currentEntry = catalogModel(models, explicitCurrentModel) || (explicitCurrentModel
    ? { id: normalizeCodexModel(explicitCurrentModel), supportedReasoningEfforts: [], serviceTiers: [] }
    : defaultEntry);
  ...
  if (command === "effort") {
    if (!tokens.length || (tokens.length === 1 && tokens[0].toLowerCase() === "status")) {
      return { ok: true, action: "status", replyText: `Model: ${currentEntry?.id || "unavailable"} · effort: ${currentEffort || "default"}\nSupported efforts: ${supportedEfforts(currentEntry || { supportedReasoningEfforts: [] }).join(", ") || "not reported"}` };
    }
  ...
  if (command === "fast") {
    const alias = (tokens[0] || "status").toLowerCase();
    const action = ({ enable: "on", enabled: "on", disable: "off", disabled: "off" })[alias] || alias;
    if (tokens.length > 1 || !["toggle", "on", "off", "status"].includes(action)) {
      return { ok: false, error: "Use /fast status, /fast enable, /fast disable, or /fast toggle." };
    }
