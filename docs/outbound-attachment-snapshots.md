...
## Shared progress and final delivery

Staging recovery and snapshot-aware attachment preparation apply to both
progress and final WhatsApp replies. A recovered progress attachment updates
only that message's delivery status; it does not acknowledge a final turn.
Snapshot obligations retained in the durable outbox remain mandatory even when
the current message no longer contains them. Required snapshot checks precede
generic attachment preflight checks, so a missing recoverable snapshot is not
silently discarded or incorrectly classified as a permanent failure.

Snapshot recovery never reopens a partial or uncertain send. Those terminal
states retain the connector's existing explicit-review/no-replay safeguards.
Implicit credential-file rejection also remains in force before source-path
deduplication, including `sandbox:` and `file:` references.
