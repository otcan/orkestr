...
| Optional Claude Code runtime | Operator-admin host only for now | Configured admin owner and non-contained thread required before creation and execution | Missing/foreign/contained owners cannot start a Claude process; tenant support remains disabled pending its sandbox contract |
...
| Browser desktops | Tenant-owned browser profile and, for external accounts, private attested tenant/desktop isolation | Per-user browser profile roots, exact thread grants, exclusive fenced leases, short-lived single-use broker capabilities, endpoint redaction, and owner-scoped share links | User A cannot list, lease, restart, share, or broker access to User B desktop; stale, replayed, cross-thread, cross-runtime, and cross-resource capabilities fail closed |
