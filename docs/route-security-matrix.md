...
| Tenant VM registry, provisioning, and WA routes | `GET/POST/PATCH/DELETE /api/tenant-vms`, `POST /api/tenant-vms/:id/provision`, `POST/DELETE /api/tenant-vms/:id/whatsapp-route` | Admin only | tenant VM registry/provisioning/routing controller and control-plane guard |
