...
export function legalPage(pageId: PublicPageId, env = process.env): PublicPage {
  ...
  const pages: Partial<Record<PublicPageId, PublicPage>> = {
    ...
    terms: {
      ...
      body: legalBody("TERMS", "Terms", "Orkestr includes public-alpha software and managed implementation services. The personal beta is invite-only, and submitting a commercial inquiry is not a purchase agreement.", [
        ...
        ["Commercial inquiries", "Submitting a Project Discovery description or Workflow Audit asks Orkestr to assess a possible managed implementation. It does not guarantee feasibility, availability, scope, price, schedule, or delivery. Any project begins only under an agreed proposal or contract."],
