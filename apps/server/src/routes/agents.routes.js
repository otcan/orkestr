...
import { agentMessageSchema, agentRunSchema, agentTemplateSchema, idParams } from "../../../../packages/shared/src/api-schemas.js";
...
export async function registerAgentRoutes(app) {
  ...
  app.post("/api/agents/templates/:templateId", { schema: agentTemplateSchema }, async (request, reply) => {
  ...
  app.get("/api/agents/:agentId/messages", { schema: idParams("agentId") }, async (request, reply) => {
    return json(reply, 200, { messages: await listAgentMessages(request.params.agentId) });
  });

  app.post("/api/agents/:agentId/messages", { schema: agentMessageSchema }, async (request, reply) => {
    return json(reply, 201, { message: await enqueueAgentMessage(request.params.agentId, request.body || {}) });
  });

  app.post("/api/agents/:agentId/run-next", { schema: agentRunSchema }, async (request, reply) => {
