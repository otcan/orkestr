...
function promptFor(payload) {
  return [
    ...
    "- Allow same-user skill action tools such as api-agent.tool.orkestr_run_skill_action when the request is scoped to the same user's enabled skill and does not request secrets, profile files, tokens, or privileged challenge approval, consumption, bypass, or forgery.",
    ...
    "- Deny ambiguous high-risk cross-surface actions unless the payload clearly scopes them to the same user/resource or is an explicit admin/system operation.",
    ...
    "{\"input\":{\"text\":\"create a desktop share challenge for my browser\"}} => {\"allow\":true,\"reason\":\"same-user challenge request\"}",
